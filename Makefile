...
LD_FLAGS                    := "-w -X github.com/metal-stack/gardener-extension-ontap/pkg/version.Version=$(IMAGE_TAG)"
VERIFY                      := true
...
.PHONY: build
build:
	go build -ldflags $(LD_FLAGS) -tags netgo -o bin/gardener-extension-ontap ./cmd/gardener-extension-ontap
...
docker-image:
	@docker build --no-cache \
		...
		--tag $(IMAGE_PREFIX)/gardener-extension-ontap:$(IMAGE_TAG) \
		--file Dockerfile --memory 6g .

.PHONY: docker-push
docker-push:
	@docker push $(IMAGE_PREFIX)/gardener-extension-ontap:$(IMAGE_TAG)
...
generate-in-docker: tidy install $(HELM) $(YQ)
	docker run --rm -i$(DOCKER_TTY_ARG) -v $(PWD):/go/src/github.com/metal-stack/gardener-extension-ontap golang:$(GO_VERSION) \
		sh -c "cd /go/src/github.com/metal-stack/gardener-extension-ontap \
				&& make generate \
